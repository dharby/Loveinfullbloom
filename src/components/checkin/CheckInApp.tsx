...
const inputCls =
  "w-full h-14 px-4 text-lg rounded-md bg-white border border-#3A7D5C/20 text-#3A7D5C placeholder:text-#3A7D5C/35 outline-none focus:border-#9B8CB7";
...
export default function CheckInApp() {
  ...
  if (screen === "loading") {
    return (
      <div className="min-h-dvh bg-#FAF8FB flex items-center justify-center">
        <p className="text-#3A7D5C/60 tracking-[0.2em] text-xs uppercase">Loading…</p>
      </div>
    );
  }
  ...
  if (screen === "login") {
    return (
      <div className="min-h-dvh bg-#FAF8FB text-#3A7D5C flex flex-col overflow-x-hidden">
        <div className="flex-1 flex flex-col items-center justify-center px-6 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] w-full max-w-md mx-auto">
          <p className="text-xs uppercase tracking-[0.35em] text-#9B8CB7 mb-3">Oreoluwa &amp; Daberechukwu</p>
          <h1 className="text-3xl font-serif mb-1">Guest Check-in</h1>
          <p className="text-sm text-#3A7D5C/60 mb-8">Enter usher password</p>
          <form onSubmit={doLogin} className="w-full space-y-4">
            ...
            <button
              ...
              className="w-full h-14 rounded-md bg-#3A7D5C text-#FAF8FB text-sm uppercase tracking-[0.2em] font-medium border border-#9B8CB7/40 disabled:opacity-50 touch-manipulation active:scale-[0.99]"
            >
  ...
  const header = (
    <div className="pt-[env(safe-area-inset-top)] bg-#3A7D5C text-#FAF8FB border-b border-#9B8CB7/40">
      <div className="px-5 pt-4 pb-3 flex items-center justify-between max-w-md mx-auto w-full">
        <div>
          <p className="text-[0.6rem] uppercase tracking-[0.3em] text-#9B8CB7">Oreoluwa &amp; Daberechukwu</p>
          <p className="text-lg font-serif">Guest Check-in</p>
        </div>
        <button
          onClick={doLogout}
          className="h-10 px-4 rounded-md border border-#9B8CB7/50 text-xs uppercase tracking-[0.15em] text-#FAF8FB/90 touch-manipulation active:scale-[0.97]"
        >
          Log out
        </button>
      </div>
    ...
    return (
      <div className="min-h-dvh bg-#FAF8FB text-#3A7D5C flex flex-col overflow-x-hidden">
        ...
        <div className="flex-1 w-full max-w-md mx-auto px-5 py-6 pb-[env(safe-area-inset-bottom)]">
          <p className="text-xs uppercase tracking-[0.25em] text-#3A7D5C/60 mb-4 text-center">
            ...
            {CHECKIN_CATEGORIES.map((c) => {
              ...
              return (
                <button
                  ...
                  className="w-full min-h-[5.5rem] p-5 rounded-lg bg-#3A7D5C text-#FAF8FB border border-#9B8CB7/50 text-left touch-manipulation active:scale-[0.99]"
                >
                  <span className="block text-base font-medium uppercase tracking-[0.12em]">{c.label}</span>
                  <span className="block mt-1 text-sm text-#FAF8FB/70">
          ...
          {counts && (
            <button
              ...
              className="mt-5 p-4 rounded-lg bg-white border border-#3A7D5C/10 text-center w-full touch-manipulation active:scale-[0.99]"
            >
              <p className="text-xs uppercase tracking-[0.2em] text-#3A7D5C/60">Total checked in</p>
              <p className="text-3xl font-serif mt-1">{counts.totalCheckedIn}</p>
              <p className="text-xs text-#9B8CB7 mt-1 uppercase tracking-wider">View all guests ›</p>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="min-h-dvh bg-#FAF8FB text-#3A7D5C flex flex-col overflow-x-hidden">
        ...
        <div className="flex-1 w-full max-w-md mx-auto px-5 py-4 pb-[env(safe-area-inset-bottom)]">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-#3A7D5C/60">All checked-in guests</p>
              <p className="text-2xl font-serif mt-0.5">{checkedTotal}</p>
            </div>
            <button
              onClick={() => { setScreen("categories"); loadCounts(); }}
              className="h-10 px-4 rounded-md border border-#3A7D5C/20 text-xs uppercase tracking-[0.15em] text-#3A7D5C/70 touch-manipulation active:scale-[0.97]"
            >
              Back
            </button>
          </div>
          <div className="flex gap-2 mb-4">
            ...
            <button
              ...
              className="shrink-0 h-14 px-5 rounded-md bg-#3A7D5C text-#FAF8FB text-sm uppercase tracking-[0.12em] font-medium border border-#9B8CB7/40 disabled:opacity-50 touch-manipulation active:scale-[0.98]"
            >
              {checkedLoading ? "…" : "Go"}
            </button>
          </div>
          <button
            onClick={() => loadCheckedList(checkedQuery)}
            className="w-full text-right text-xs text-#9B8CB7 uppercase tracking-wider mb-3"
          >
            Refresh
          </button>
          {checkedList.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-#3A7D5C/40 text-sm">No guests checked in yet</p>
            </div>
          ) : (
            <div className="space-y-2">
              {checkedList.map((g) => (
                <div
                  key={g.id}
                  className="p-4 rounded-lg bg-white border border-#3A7D5C/15"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-base font-medium truncate">{g.name}</p>
                      <p className="text-xs uppercase tracking-[0.15em] text-#9B8CB7 mt-0.5">
                        {checkinCategoryLabel(g.category)}
                      </p>
                    </div>
                    <span className="shrink-0 text-xs font-medium text-primary bg-primary-soft border border-primary-soft rounded-full px-2.5 py-1">
                      ✓ In
                    </span>
                  </div>
                  <div className="mt-2 text-xs text-#3A7D5C/60 space-y-0.5">
  ...
  return (
    <div className="min-h-dvh bg-#FAF8FB text-#3A7D5C flex flex-col overflow-x-hidden">
      ...
      <div className="bg-#3A7D5C text-#FAF8FB border-t border-#9B8CB7/20">
        <div className="px-5 py-3 max-w-md mx-auto w-full flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[0.6rem] uppercase tracking-[0.25em] text-#9B8CB7">Guest category</p>
            <p className="text-base font-medium truncate">{activeLabel}</p>
            <p className="text-sm text-#FAF8FB/70">
          ...
          <button
            ...
            className="shrink-0 h-11 px-4 rounded-md border border-#9B8CB7/60 text-xs uppercase tracking-[0.15em] touch-manipulation active:scale-[0.97]"
          >
            Change
          </button>
        </div>
      ...
      <div className="px-5 pt-4 max-w-md mx-auto w-full">
        <div className="grid grid-cols-3 gap-1.5 p-1 rounded-lg bg-#3A7D5C/5 border border-#3A7D5C/10">
          {(["search", "scan", "checked"] as DeskTab[]).map((t) => (
            <button
              ...
              className={`h-12 rounded-md text-xs uppercase tracking-[0.1em] font-medium touch-manipulation ${
                tab === t ? "bg-#3A7D5C text-#FAF8FB" : "text-#3A7D5C/60"
              }`}
            >
              ...
              <div className="mt-4 space-y-2">
                {results.map((g) => (
                  <button
                    ...
                    className="w-full p-4 rounded-lg bg-white border border-#3A7D5C/15 text-left touch-manipulation active:scale-[0.99]"
                  >
                    <span className="flex items-center justify-between gap-2">
                      ...
                      {g.checkInStatus === "checked_in" ? (
                        ...
                        <span className="shrink-0 text-xs uppercase tracking-wider text-#3A7D5C/50">View guest ›</span>
                      )}
                    </span>
                    <span className="block mt-1 text-xs uppercase tracking-[0.15em] text-#9B8CB7">
                      {checkinCategoryLabel(g.category)}
                    </span>
                    <span className="block mt-0.5 text-xs text-#3A7D5C/60">
        ...
        {tab === "scan" && !detail && (
          <div>
            {!cameraBlocked ? (
              ...
              <div className="p-5 rounded-lg bg-white border border-#3A7D5C/15 text-center">
                <p className="font-medium">Camera unavailable</p>
                <p className="mt-1 text-sm text-#3A7D5C/60">
                  Please allow camera access, or search for the guest instead.
                </p>
                <button
                  onClick={() => setTab("search")}
                  className="mt-4 w-full h-13 min-h-[3.25rem] rounded-md bg-#3A7D5C text-#FAF8FB text-sm uppercase tracking-[0.15em] font-medium touch-manipulation active:scale-[0.99]"
                >
                  Search guest instead
                </button>
              </div>
            )}
            <p className="mt-3 text-center text-xs text-#3A7D5C/50">
        ...
        {tab === "checked" && !detail && (
          <div>
            ...
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs uppercase tracking-[0.2em] text-#3A7D5C/60">
                {checkedTotal} guest{checkedTotal !== 1 ? "s" : ""} checked in
              </p>
              <button
                onClick={() => loadCheckedList(checkedQuery, category || undefined)}
                className="text-xs text-#9B8CB7 uppercase tracking-wider"
              >
                Refresh
              </button>
            </div>
          ...
          <div className="rounded-lg bg-white border border-#3A7D5C/15 p-5">
            ...
            {detail.checkInStatus === "checked_in" ? (
              ...
              <button
                onClick={doCheckIn}
                disabled={checkingIn}
                className="mt-5 w-full h-14 rounded-md bg-#3A7D5C text-#FAF8FB text-sm uppercase tracking-[0.2em] font-medium border border-#9B8CB7/40 disabled:opacity-60 touch-manipulation active:scale-[0.99]"
              >
            ...
            <button
              onClick={backToDesk}
              className="mt-2 w-full h-12 rounded-md border border-#3A7D5C/20 text-sm uppercase tracking-[0.15em] text-#3A7D5C/70 touch-manipulation active:scale-[0.99]"
            >
              Back to check-in
            </button>
          </div>
      ...
      {overlay?.kind === "success" && (
        <div className="fixed inset-0 z-50 bg-#3A7D5C text-#FAF8FB flex flex-col items-center justify-center px-8 text-center">
          <span className="w-16 h-16 rounded-full bg-#9B8CB7 text-#3A7D5C text-3xl flex items-center justify-center">
            ✓
          </span>
          <p className="mt-5 text-xs uppercase tracking-[0.3em] text-#9B8CB7">Checked in</p>
          <p className="mt-2 text-3xl font-serif">{overlay.guest.name}</p>
          <p className="mt-1 text-sm text-#FAF8FB/70">{checkinCategoryLabel(overlay.guest.category)}</p>
          {overlay.time && (
            <p className="mt-1 text-sm text-#FAF8FB/70">Checked in at {formatTime(overlay.time)}</p>
          )}
        </div>
      )}

      {overlay?.kind === "already" && (
        <div className="fixed inset-0 z-50 bg-#3A7D5C/97 text-#FAF8FB flex flex-col items-center justify-center px-8 text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-#9B8CB7">✓ Already checked in</p>
          <p className="mt-2 text-3xl font-serif">{overlay.guest.name}</p>
          <p className="mt-1 text-sm text-#FAF8FB/70">{checkinCategoryLabel(overlay.guest.category)}</p>
          {overlay.guest.checkInTime && (
            <p className="mt-1 text-sm text-#FAF8FB/70">Checked in at {formatTime(overlay.guest.checkInTime)}</p>
          )}
          <button
            onClick={backToDesk}
            className="mt-6 w-full max-w-xs h-13 min-h-[3.25rem] rounded-md border border-#9B8CB7/60 text-sm uppercase tracking-[0.15em] touch-manipulation active:scale-[0.99]"
          >
            Back to check-in
          </button>
        </div>
      ...
      {overlay?.kind === "mismatch" && (
        <div className="fixed inset-0 z-50 bg-#3A7D5C/97 text-#FAF8FB flex flex-col items-center justify-center px-8 text-center overflow-y-auto">
          <p className="text-xs uppercase tracking-[0.3em] text-red-300">Category mismatch</p>
          <p className="mt-3 text-sm text-#FAF8FB/80">This guest belongs to:</p>
          <p className="mt-1 text-xl font-medium text-#9B8CB7">
            {checkinCategoryLabel(overlay.actualCategory)}
          </p>
          <p className="mt-3 text-sm text-#FAF8FB/80">but you are currently checking in:</p>
          <p className="mt-1 text-xl font-medium">{category ? checkinCategoryLabel(category) : ""}</p>
          <p className="mt-3 text-sm text-#FAF8FB/70">Please select the correct guest category.</p>
          <button
            onClick={() => {
              setOverlay(null);
              setDetail(null);
            ...
            className="mt-6 w-full max-w-xs h-13 min-h-[3.25rem] rounded-md bg-#9B8CB7 text-#3A7D5C text-sm uppercase tracking-[0.15em] font-medium touch-manipulation active:scale-[0.99]"
          >
            Change category
          </button>
          <button
            onClick={backToDesk}
            className="mt-2 w-full max-w-xs h-12 rounded-md border border-#FAF8FB/30 text-sm uppercase tracking-[0.15em] text-#FAF8FB/80 touch-manipulation active:scale-[0.99]"
          >
            Back
          </button>
        </div>
      ...
      {overlay?.kind === "notfound" && (
        <div className="fixed inset-0 z-50 bg-#3A7D5C/97 text-#FAF8FB flex flex-col items-center justify-center px-8 text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-#9B8CB7">Guest not found</p>
          <p className="mt-3 text-sm text-#FAF8FB/80 max-w-xs">{overlay.message}</p>
          <button
            onClick={backToDesk}
            className="mt-6 w-full max-w-xs h-13 min-h-[3.25rem] rounded-md bg-#9B8CB7 text-#3A7D5C text-sm uppercase tracking-[0.15em] font-medium touch-manipulation active:scale-[0.99]"
          >
